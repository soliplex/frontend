// A COPY of lib/src/modules/room/ui/room_screen.dart for the mockup harness.
// Edit freely — the app's screen is untouched. Leaf widgets (composer,
...
import '../../modules/room/anchor_tracker.dart';
import '../../modules/room/room_run_activity.dart';
import '../../modules/room/room_unread.dart';
import '../../modules/room/thread_anchor_storage.dart';
import '../../modules/room/thread_read_markers.dart';
import '../../modules/room/thread_read_tracker.dart';
import '../../modules/room/unread_boundary.dart';
import '../../modules/room/document_filter_hydration.dart';
import '../../modules/room/document_selections.dart';
import '../../modules/room/pick_file.dart';
import '../../modules/room/agent_runtime_manager.dart';
import '../../modules/room/room_state.dart';
...
import '../../modules/room/ui/chat_classification.dart';
import 'chat_input.dart';
import 'thread_panel/panel_sample_data.dart';
import 'thread_panel/thread_panel.dart';
import '../../modules/room/ui/chunk_visualization_page.dart';
import '../../modules/room/ui/copy_button.dart';
import '../../modules/room/ui/document_picker.dart';
import '../../modules/room/ui/error_retry_panel.dart';
import '../../modules/room/ui/feedback_reason_dialog.dart';
import '../../modules/room/ui/inline_image_composer_controller.dart';
import '../../modules/room/ui/message_timeline.dart';
import '../../modules/room/ui/async_action_dialog.dart';
...
class _RoomScreenState extends State<RoomScreen> {
  ...
  // MOCKUP: the floating thread panel's stubbed state — databases, skills,
  // context usage, workspace draft. One per screen; nothing here is wired.
  final _panel = samplePanelState();
  ...
  void initState() {
    super.initState();
    // MOCKUP: the composer's ring reads the panel's usage, so a change
    // there (a database connected, a skill loaded) has to reach it.
  ...
  ({Widget content, List<Widget> headerActions}) _buildContent(
    ...
    // MOCKUP: the wide layout is the one whose header is in the page; on
    // the narrow one the panel cannot float and opens as a sheet instead.
    final panelFloats = showHeader;
    ...
    return (
      content: LayoutBuilder(
        builder: (context, constraints) {
          // MOCKUP: the conversation column and the composer move over so
          // the open panel sits beside them rather than on them, as long as
          // the column keeps a tablet's width; narrower than that, the panel
          // floats over the column. The composer follows the column so the
          // two stay aligned.
          final reserve = panelFloats &&
                  threadView != null &&
                  _panel.isOpen &&
                  constraints.maxWidth >=
                      SoliplexBreakpoints.tablet +
                          ThreadPanel.preferredWidth +
                          SoliplexSpacing.s3 * 2
              ? ThreadPanel.preferredWidth + SoliplexSpacing.s3
              : 0.0;
          Widget beside(Widget child) => Padding(
                padding: EdgeInsets.only(right: reserve),
                child: _capWidth(child),
              );
          return Column(
            children: [
              StatusMessageBanner(
              ...
              Expanded(
                child: Stack(
                  children: [
                    beside(body),
                    // MOCKUP: the thread panel floats over the chat area, top
                    // right, so it reads as the thread's — the rail, sidebar
                    // and header are the room's. Only a thread has one.
                    if (panelFloats && threadView != null)
                      Positioned.fill(
                        child: Padding(
                          padding: const EdgeInsets.all(SoliplexSpacing.s3),
                          child: Align(
                            alignment: Alignment.topRight,
                            child: ThreadPanel(state: _panel),
                          ),
                        ),
                      ),
                  ],
              ...
              beside(
                _buildChatInput(
                  ...
                  room,
                  messagesStatus,
                  openThreadPanel: !panelFloats && threadView != null
                      ? () => showThreadPanelSheet(context, state: _panel)
                      : null,
                ),
              ),
              // Last in the column so the caveat is the last thing read before
              // sending.
              beside(ChatAiDisclaimer(appName: widget.appName)),
            ],
          );
        },
      ),
  ...
  Widget _buildChatInput(
    ...
    return ChatInput(
      ...
      // MOCKUP: the filter is stubbed until its purpose is settled — RAG
      // scope, embeds, or something else — so the item shows but only says
      // so.
      onFilterTap: _showDocumentFilter ? _stubDocumentFilter : null,
      ...
      // MOCKUP: the ring reads the panel's stubbed usage; the welcome
      // composer has no thread to measure, so it shows none.
      contextUsage: threadView == null ? null : _panel.contextUsage,
      onOpenThreadPanel: openThreadPanel,
    );
  }

  void _stubDocumentFilter() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Document filtering — stubbed until its scope is settled '
          '(RAG, embeds, or both).',
        ),
      ),
    );
