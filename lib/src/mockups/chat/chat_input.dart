...
import '../../modules/room/attachable_images.dart';
import '../../modules/room/pick_file.dart' show PickFileException;
import '../../modules/room/pick_image.dart';
import '../../modules/room/ui/document_label.dart';
...
class ChatInput extends StatefulWidget {
  ...
  /// MOCKUP: opens the thread panel where it cannot float beside the
  /// conversation. Null where it can — the floating panel has its own
  /// handle there.
  final VoidCallback? onOpenThreadPanel;

  @override
  State<ChatInput> createState() => _ChatInputState();
}

/// MOCKUP: what the single `+` offers.
enum _AddChoice { attachFiles, attachFolder, filter }
...
class _ChatInputState extends State<ChatInput> {
  ...
  /// MOCKUP: the composer's leading controls, reduced to one `+` menu —
  /// attach (files, or a folder), and the document filter — plus, where the
  /// thread panel cannot float, the button that opens it as a sheet.
  ///
  /// Add-image is folded into attach: the app can pick the path from the
  /// file's type, inline for an image and upload for anything else. That
  /// routing lives in the picker plumbing the callbacks own, so here attach
  /// takes the upload path for every file.
  List<Widget> _leadingControls(
    ...
    return [
      PopupMenuButton<_AddChoice>(
        ...
        itemBuilder: (_) => [
          const PopupMenuItem(
            value: _AddChoice.attachFiles,
            child: ListTile(
              leading: Icon(Icons.attach_file),
              title: Text('Attach files or images…'),
            ),
          ),
          if (widget.onAttachFolder != null)
            const PopupMenuItem(
              value: _AddChoice.attachFolder,
              child: ListTile(
                leading: Icon(Icons.folder_open),
                title: Text('Attach folder…'),
              ),
            ),
          if (widget.onFilterTap != null)
            PopupMenuItem(
              value: _AddChoice.filter,
              child: ListTile(
                leading: Icon(
                  Icons.filter_alt,
        ...
        onSelected: _onAddChoice,
      ),
      if (widget.onOpenThreadPanel case final open?)
        IconButton(
          icon: const Icon(Icons.tune),
          tooltip: 'Thread panel',
          onPressed: open,
        ),
    ];
  }

  /// MOCKUP: runs a `+` choice. Everything the choice needs is read here
  /// rather than when the menu was filled, and a composer handed to a run
  /// meanwhile absorbs the tap. A room with no upload affordance still gets
  /// the inline-image path from the same item.
  void _onAddChoice(_AddChoice choice) {
    if (!_acceptsInput) return;
    switch (choice) {
      case _AddChoice.attachFiles:
        if (widget.onAttachFile case final attach?) {
          attach();
        } else {
          _addImages();
        }
      case _AddChoice.attachFolder:
        widget.onAttachFolder?.call();
      case _AddChoice.filter:
