...
Widget _gallery({required Brightness brightness}) => MaterialApp(
      ...
      home: const Scaffold(
        body: SingleChildScrollView(
          ...
          child: EffectsGallery(),
        ),
      ),
    );

void main() {
  testWidgets('EffectsGallery — light', (tester) async {
    tester.view.physicalSize = const Size(700, 700);
    tester.view.devicePixelRatio = 1;
    addTearDown(tester.view.reset);

    await tester.pumpWidget(_gallery(brightness: Brightness.light));
    // The sweep loops forever; advance to a fixed mid-cycle frame (half of the
    // 1600ms period) so the snapshot is deterministic.
    await tester.pump(const Duration(milliseconds: 800));

    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('goldens/effects_gallery_light.png'),
    );
  });

  testWidgets('EffectsGallery — dark', (tester) async {
    tester.view.physicalSize = const Size(700, 700);
    tester.view.devicePixelRatio = 1;
    addTearDown(tester.view.reset);
    ...
    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('goldens/effects_gallery_dark.png'),
    );
  });
}
